...
std::unique_ptr<CircleQuantParam> luci_quantparam(const circle::QuantizationParameters *qparams)
{
  ...
  assert(qparams);
  auto quantization = qparams->UnPack();

  bool minmax_exist = not quantization->min.empty() && not quantization->max.empty();
  bool scale_and_zp_exist = not quantization->scale.empty() && not quantization->zero_point.empty();
  if (minmax_exist || scale_and_zp_exist)
  {
    return nullptr;
  }

  // move quantization parameters to result
  auto quantparam = std::make_unique<CircleQuantParam>();
  quantparam->min = std::move(quantization->min);
  quantparam->max = std::move(quantization->max);
  quantparam->scale = std::move(quantization->scale);
  quantparam->zerop = std::move(quantization->zero_point);
  quantparam->quantized_dimension = quantization->quantized_dimension;
...
std::unique_ptr<SparsityParam> luci_sparsityparam(const circle::SparsityParameters *sparparam)
{
  // create temporary unpacked API object
  assert(sparparam);
  auto sparsity = sparparam->UnPack();

  // TODO find a condition that should return nullptr
  auto sparsityparam = std::make_unique<SparsityParam>();

  sparsityparam->traversal_order = std::move(sparsity->traversal_order);
  sparsityparam->block_map = std::move(sparsity->block_map);
...
void copy_tensor_attributes(const circle::Tensor *tensor, CircleNode *node)
{
  ...
  auto const dims = tensor->shape(); // in NHWC
  node->rank(dims->size());
  for (uint32_t r = 0; r < dims->size(); ++r)
  {
    if (tensor->shape_signature()->size() > 0 && tensor->shape_signature()->Get(r) == -1)
      node->dim(r).unset();
    else
      node->dim(r).set(dims->Get(r));
  }
...
template <typename T> typename VectorWrapper<T>::return_type VectorWrapper<T>::at(uint32_t i) const
{
  if (i >= size())
  {
...
template <typename T>
typename VectorWrapper<T>::return_type VectorWrapper<T>::operator[](uint32_t i) const
{
  return at(i);
}
