...
TEST(CircleConstProxyTest, constructor)
{
 luci_micro::CircleConstProxy node;

 ASSERT_EQ(luci::CircleDialect::get(), node.dialect());
 ASSERT_EQ(luci::CircleOpcode::CIRCLECONST, node.opcode());
}

TEST(CircleConstProxyTest, value_setting)
{
 luci_micro::CircleConstProxy  node;
...
TEST(CircleConstProxyTest, scalar)
{
 luci_micro::CircleConstProxy  node;
 node.dtype(loco::DataType::S32);
