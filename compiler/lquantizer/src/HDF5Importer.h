...
#ifndef __LQUANTIZER_HDF5IMPORTER_H__
#define __LQUANTIZER_HDF5IMPORTER_H__
...
{
public:
  HDF5Importer() = default;

  explicit HDF5Importer(const std::string &path) { open_file(path); }

public:
  /**
   * @brief Open hdf5 file. If file not in h5 format function will throw runtime_error.
   */
  void open_file(const std::string &path)
  {
    if (not H5::H5File::isHdf5(path))
...
#endif // __LQUANTIZER_HDF5IMPORTER_H__
