macro(initialize_pal)
    ...
    if (NOT TensorFlowSource_FOUND)
        message(STATUS "Skipping micro: TensorFlow not found")
        return()
    endif ()

    if (NOT TensorFlowGEMMLowpSource_FOUND)
        message(STATUS "Skipping micro: gemmlowp not found")
        return()
    endif ()

    if (NOT TensorFlowEigenSource_FOUND)
        message(STATUS "Skipping micro: Eigen not found")
        return()
    endif ()

    if (NOT TensorFlowRuySource_FOUND)
        message(STATUS "Skipping micro: Ruy not found")
        return()
    endif ()
...
macro(add_pal_to_target TGT)
    ...
    target_include_directories(${TGT} PRIVATE ${MICRO_PAL_DIR})
    ...
    add_library(micro_mcu_pal STATIC ${PAL_SOURCES})
    set_target_properties(micro_mcu_pal PROPERTIES POSITION_INDEPENDENT_CODE ON)
    target_include_directories(micro_mcu_pal SYSTEM PRIVATE "${TensorFlowSource_DIR}")

    target_link_libraries(${TGT} PRIVATE micro_mcu_pal)
    #target_link_libraries(${TGT} PRIVATE Threads::Threads micro_mcu_pal)
endmacro()
