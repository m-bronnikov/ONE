...
void DepthwiseConv2D::configure()
{
  ...
  else if (input()->element_type() == DataType::S8 && filter()->element_type() == DataType::S8)
  {
    ...
    MICRO_CHECK(static_cast<uint32_t>(filter()->shape().dim(3)) == filter()->scales().size());
  ...
  MICRO_CHECK(filter_shape.dim(0) == 1);
...
void DepthwiseConv2D::execute() const
{
  switch (input()->element_type())
  {
    ...
    case DataType::U8:
      ...
      {
        MICRO_CHECK(filter()->shape().num_dims() == 4);
        MICRO_CHECK(filter()->scales().size() == static_cast<size_t>(filter()->shape().dim(3)));
