...
void Pack::configure()
{
  MICRO_CHECK(_inputs.size() == static_cast<uint32_t>(params().values_count));
  ...
  MICRO_CHECK(axis >= 0 && axis <= t0->shape().num_dims());
  ...
  for (uint32_t i = 1; i < _inputs.size(); ++i)
  {
    ...
    MICRO_CHECK(tensor->element_type() == t0->element_type());
    MICRO_CHECK(tensor->shape().num_dims() == t0->shape().num_dims());
    for (int d = 0; d < t0->shape().num_dims(); ++d)
    {
      MICRO_CHECK(tensor->shape().dim(d) == t0->shape().dim(d));
    }
  }
  ...
  {
    MICRO_CHECK(output()->zero_point() == t0->zero_point());
    MICRO_CHECK(output()->scale() == t0->scale());
    // Guarantee input/output quantization params match as we do not support
    // packing quantized tensors.
    for (int i = 0; i < params().values_count; i++)
    {
      MICRO_CHECK(_inputs[i]->zero_point() == t0->zero_point());
      MICRO_CHECK(_inputs[i]->scale() == t0->scale());
    }
  }
