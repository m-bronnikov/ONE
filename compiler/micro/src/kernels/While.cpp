...
void copy(const std::vector<const Tensor *> &src, const std::vector<Tensor *> &dst)
{
  ...
  {
    MICRO_CHECK(dst[i]->element_type() == src[i]->element_type());
...
{
  MICRO_CHECK(_body_graph->getInputTensors().size() == getInputTensors().size());
  MICRO_CHECK(_body_graph->getOutputTensors().size() == getOutputTensors().size());
  MICRO_CHECK(_body_graph->getOutputTensors().size() == getInputTensors().size());

  MICRO_CHECK(_cond_graph->getInputTensors().size() == getInputTensors().size());

  const auto &cond_outputs = _cond_graph->getOutputTensors();
  MICRO_CHECK(cond_outputs.size() == 1)
  MICRO_CHECK(cond_outputs[0]->element_type() == DataType::BOOL);
}
