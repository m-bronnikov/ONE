...
void Softmax::configure()
{
  ...
  MICRO_CHECK(input()->shape().num_dims() >= 1);
  if (input()->element_type() == DataType::U8 || input()->element_type() == DataType::S8)
  {
    MICRO_CHECK(output()->zero_point() == 0);
    tflite::SoftmaxParams op_params{};
    op_params.table = _table;
    micro_pal::PopulateSoftmaxLookupTable(&op_params, input()->scale(), params().beta);
  }
...
template <typename T> void Softmax::evalQuantized() const
{
  ...
  micro_pal::InitializeParams(&op_params, input()->scale(), params().beta);
  micro_pal::Softmax(op_params, getTensorShape(input()), getTensorData<T>(input()),
