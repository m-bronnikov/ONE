...
void If::configure()
{
  MICRO_CHECK(cond()->element_type() == DataType::BOOL);
  MICRO_CHECK(cond()->shape().num_elements() == 1);

  for (RuntimeGraph *graph : {_then_graph, _else_graph})
  {
    (void)graph;
    MICRO_CHECK(graph->getInputTensors().size() == getInputTensors().size() - 1);
    MICRO_CHECK(graph->getOutputTensors().size() == getOutputTensors().size());
  }
}
...
void If::execute() const
{
  ...
  {
    MICRO_CHECK(graph_inputs[i]->element_type() == input(i)->element_type());
  ...
  {
    MICRO_CHECK(graph_outputs[i]->element_type() == output(i)->element_type());
