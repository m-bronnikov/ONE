...
void PRelu::configure()
{
  ...
  MICRO_CHECK(alpha()->element_type() == output()->element_type());
  MICRO_CHECK(input()->scales().size() <= 1);
  MICRO_CHECK(output()->scales().size() <= 1);

  if (input()->element_type() == DataType::U8)
  {
    MICRO_CHECK(alpha()->scales().size() <= 1); // remove when CWQ kernel arrives
  ...
  else if (input()->element_type() == DataType::S16)
  {
    ...
    MICRO_CHECK(input()->zero_point() == 0 && output()->zero_point() == 0);
    for (size_t channel = 0; channel < alpha()->zero_points().size(); ++channel)
    {
      MICRO_CHECK(alpha()->zero_points()[channel] == 0);
    }
    // PRelu specific checks for CWQ
    MICRO_CHECK(alpha()->quantized_dimension() == alpha()->shape().num_dims() - 1);
    MICRO_CHECK(static_cast<int32_t>(alpha()->scales().size()) ==
                alpha()->shape().dim(alpha()->quantized_dimension()));
    MICRO_CHECK(alpha()->shape().num_elements() ==
    ...
    {
      MICRO_CHECK(alpha()->shape().dim(dim) == 1);
    }
