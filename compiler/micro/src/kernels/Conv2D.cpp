...
void Conv2D::configure()
{
  ...
  {
    MICRO_CHECK(bias() == nullptr || bias()->element_type() == DataType::FLOAT32);
  }
  else if (input()->element_type() == DataType::U8 && filter()->element_type() == DataType::U8)
  {
    MICRO_CHECK(bias() == nullptr || bias()->element_type() == DataType::S32);
  }
  else if (input()->element_type() == DataType::S8 && filter()->element_type() == DataType::S8)
  {
    MICRO_CHECK(bias() == nullptr || bias()->element_type() == DataType::S32);
    MICRO_CHECK(filter()->shape().num_dims() == 4);
    MICRO_CHECK(filter()->scales().size() == static_cast<size_t>(filter()->shape().dim(0)));
  ...
  {
    MICRO_CHECK(bias() == nullptr || bias()->element_type() == DataType::S64);
  }
  else
  {
    throw std::runtime_error("Unsupported type.");
  }
  MICRO_CHECK(output()->element_type() == input()->element_type());

  const Shape &input_shape = input()->shape();
  const Shape &filter_shape = filter()->shape();
  MICRO_CHECK(input_shape.num_dims() == 4 && filter_shape.num_dims() == 4);
  ...
  MICRO_CHECK(filter_shape.dim(3) == input_shape.dim(3));

  MICRO_CHECK(bias() == nullptr ||
...
void Conv2D::evalFloat() const
{
  ...
  micro_pal::Conv(params, getTensorShape(input()), getTensorData<float>(input()),
                  getTensorShape(filter()), getTensorData<float>(filter()), getTensorShape(bias()),
                  getTensorData<float>(bias()), getTensorShape(output()),
                  getTensorData<float>(output()), getTensorShape(im2col), im2col_data);
}
...
void Conv2D::evalQuantized() const
{
  ...
  auto im2col = getOutputTensors()[1];
  micro_pal::Conv(params, getTensorShape(input()), getTensorData<uint8_t>(input()),
...
void Conv2D::evalQuantizedS8PerChannel() const
{
  ...
  micro_pal::ConvPerChannel(params, multipliers.data(), shifts.data(), getTensorShape(input()),
