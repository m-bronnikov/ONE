...
void ResizeBilinear::configure()
{
  ...
  MICRO_CHECK(size()->shape().num_dims() == 1);
  MICRO_CHECK(size()->element_type() == DataType::S32);
  if (params().half_pixel_centers && params().align_corners)
    throw std::runtime_error("If half_pixel_centers is True, align_corners must be False.");
  MICRO_CHECK(size()->shape().dim(0) == 2);
...
void ResizeBilinear::execute() const
{
  ...
  switch (output()->element_type())
  {
    case DataType::FLOAT32:
      micro_pal::ResizeBilinear(op_params, getTensorShape(input()), getTensorData<float>(input()),
                                getTensorShape(size()), getTensorData<int32_t>(size()),
                                getTensorShape(output()), getTensorData<float>(output()));
      break;
    case DataType::U8:
      micro_pal::ResizeBilinear(op_params, getTensorShape(input()), getTensorData<uint8_t>(input()),
                                getTensorShape(size()), getTensorData<int32_t>(size()),
