...
void FullyConnected::configure()
{
  ...
  {
    MICRO_CHECK(input()->element_type() == DataType::U8);
    MICRO_CHECK(output()->element_type() == DataType::U8);
    MICRO_CHECK(!bias() || bias()->element_type() == DataType::S32)
  }
  else if (weights()->element_type() == DataType::FLOAT32)
  {
    MICRO_CHECK(input()->element_type() == DataType::FLOAT32);
    MICRO_CHECK(output()->element_type() == DataType::FLOAT32);
    MICRO_CHECK(!bias() || bias()->element_type() == DataType::FLOAT32)
  }
  else if (weights()->element_type() == DataType::S8)
  {
    MICRO_CHECK(input()->element_type() == DataType::S8);
    MICRO_CHECK(output()->element_type() == DataType::S8);
    MICRO_CHECK(!bias() || bias()->element_type() == DataType::S32)
  }
  else
  {
  ...
  MICRO_CHECK(weights_shape.num_dims() == 2);
  MICRO_CHECK(bias() == nullptr || bias()->shape().num_elements() == weights_shape.dim(0));

  MICRO_CHECK(input_shape.num_elements() % weights_shape.dim(1) == 0);
  const int32_t batch_size = input_shape.num_elements() / weights_shape.dim(1);
  const int32_t num_units = weights_shape.dim(0);

  if (bias())
    MICRO_CHECK(bias()->shape().num_elements() == weights()->shape().dim(0));
