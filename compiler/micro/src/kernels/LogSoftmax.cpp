...
void LogSoftmax::configure()
{
  ...
  {
    MICRO_CHECK(output()->scale() == 16. / 256);
    MICRO_CHECK(output()->zero_point() == 255);

    tflite::SoftmaxParams params{};

    params.table = _table;
    params.beta = 1.0;
    micro_pal::PopulateSoftmaxLookupTable(&params, input()->scale(), params.beta);
  }
...
void LogSoftmax::evalQuantized() const
{
  ...
  micro_pal::InitializeParams(&params, input_scale, beta);
  micro_pal::LogSoftmax(params, input_scale, input_shape, input_data, output_shape, output_data);
}
