...
void SpaceToDepth::execute() const
{
  ...
  switch (input()->element_type())
  {
    case DataType::FLOAT32:
      micro_pal::SpaceToDepth(op_params, getTensorShape(input()), getTensorData<float>(input()),
                              getTensorShape(output()), getTensorData<float>(output()));
      break;
    case DataType::U8:
      micro_pal::SpaceToDepth(op_params, getTensorShape(input()), getTensorData<uint8_t>(input()),
