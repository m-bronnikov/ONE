...
void SpaceToBatchND::configure()
{
  ...
  MICRO_CHECK(input()->shape().num_dims() >= kInputMinDimensionNum);
  MICRO_CHECK(input()->shape().num_dims() <= kInputMaxDimensionNum);
  MICRO_CHECK(input()->element_type() == output()->element_type());

  int spatial_dims_num = input()->shape().num_dims() - 2;

  MICRO_CHECK(block_shape()->shape().num_dims() == 1);
  MICRO_CHECK(block_shape()->shape().dim(0) == spatial_dims_num);

  MICRO_CHECK(paddings()->shape().num_dims() == 2);
  MICRO_CHECK(paddings()->shape().dim(0) == spatial_dims_num);
  MICRO_CHECK(paddings()->shape().dim(1) == 2);
  ...
  for (int i = 0; i < spatial_dims_num; ++i)
  {
    ...
    MICRO_CHECK(final_dim_size % block_shape_data[i] == 0);
...
void SpaceToBatchND::execute() const
{
  switch (input()->element_type())
  {
    ...
    case DataType::FLOAT32:
      ...
      micro_pal::SpaceToBatchND(op_params, getTensorShape(input()), getTensorData<float>(input()),
                                getTensorShape(block_shape()),
                                getTensorData<int32_t>(block_shape()), getTensorShape(paddings()),
                                getTensorData<int32_t>(paddings()), getTensorShape(output()),
                                getTensorData<float>(output()));
      break;
    case DataType::U8:
      op_params.output_offset = output()->zero_point();
      micro_pal::SpaceToBatchND(op_params, getTensorShape(input()), getTensorData<uint8_t>(input()),
                                getTensorShape(block_shape()),
                                getTensorData<int32_t>(block_shape()), getTensorShape(paddings()),
                                getTensorData<int32_t>(paddings()), getTensorShape(output()),
