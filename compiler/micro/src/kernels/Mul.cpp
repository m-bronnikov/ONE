...
void Mul::configure()
{
  MICRO_CHECK(input1()->element_type() == input2()->element_type());
  MICRO_CHECK(output()->element_type() == input1()->element_type());
  if (input1()->element_type() == DataType::S16)
  {
    MICRO_CHECK(input1()->zero_point() == 0 && input2()->zero_point() == 0 &&
                output()->zero_point() == 0);
  }
...
void Mul::evalFloat() const
{
  ...
  if (need_broadcast)
  {
    micro_pal::BroadcastMul4DSlow(params, getTensorShape(input1()), getTensorData<float>(input1()),
                                  getTensorShape(input2()), getTensorData<float>(input2()),
                                  getTensorShape(output()), getTensorData<float>(output()));
  }
  else
  {
    micro_pal::Mul(params, getTensorShape(input1()), getTensorData<float>(input1()),
                   getTensorShape(input2()), getTensorData<float>(input2()),
                   getTensorShape(output()), getTensorData<float>(output()));
  }
}
