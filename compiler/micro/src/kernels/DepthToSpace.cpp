...
{
  MICRO_CHECK(input()->shape().num_dims() == 4);
  ...
  MICRO_CHECK(input_height == output_height / block_size);
  MICRO_CHECK(input_width == output_width / block_size);
  MICRO_CHECK(input_channels == output_channels * block_size * block_size);
...
void DepthToSpace::execute() const
{
  ...
  switch (input()->element_type())
  {
    case DataType::FLOAT32:
      micro_pal::DepthToSpace(op_params, getTensorShape(input()), getTensorData<float>(input()),
                              getTensorShape(output()), getTensorData<float>(output()));
      break;
    case DataType::U8:
      micro_pal::DepthToSpace(op_params, getTensorShape(input()), getTensorData<uint8_t>(input()),
