...
void BatchToSpaceND::configure()
{
  ...
  MICRO_CHECK(crops()->shape().num_dims() == 2);
  MICRO_CHECK(crops()->shape().dim(0) == spatial_dims_num);
  MICRO_CHECK(crops()->shape().dim(1) == 2);
  for (int i = 0; i < spatial_dims_num * 2; ++i)
  {
    MICRO_CHECK(crops_data[i] >= 0);
  }
  ...
  {
    MICRO_CHECK(output_batch_size % block_shape_data[i] == 0);
...
void BatchToSpaceND::execute() const
{
  switch (input()->element_type())
  {
    case DataType::FLOAT32:
      ...
      break;
    case DataType::U8:
      micro_pal::BatchToSpaceND(getTensorShape(input()), getTensorData<uint8_t>(input()),
