...
void L2Normalize::configure()
{
  ...
  {
    MICRO_CHECK(output()->scale() == (1. / 128.));
    MICRO_CHECK(output()->zero_point() == 128);
  }
  MICRO_CHECK(params().activation == Activation::NONE);
...
template <typename T> void L2Normalize::eval(int32_t zero_point) const
{
  ...
  micro_pal::L2Normalization(op_params, getTensorShape(input()), getTensorData<T>(input()),
